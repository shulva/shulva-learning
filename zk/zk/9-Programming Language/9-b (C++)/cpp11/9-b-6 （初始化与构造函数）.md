...
以下代码输出的结果是"Temperature is normal. No emergency cooling required"
```cpp
class Reactor {
public:
...
Because cpp doesn’t type check with direct initialization.
So C++ said “Meh, I’ll store 100.8 as an int,” and we possibly now have an error. 
This is commonly called a **narrowing conversion**

所以cpp11引入了uniform initialization(统一/列表初始化)
...
简单一句话概括：**允许你用一行代码，将数组、结构体、Pair 或 Tuple 中的元素，“拆解”并绑定到多个独立的变量上。**

如果你熟悉 Python ，这其实就是 C++ 版的 **“解包” (Unpacking/Destructuring)**。
...
auto [className, buildingName, language] = getClassInfo(); 
	...
	std::cout << "Come to " << buildingName << " and join us for " << className << " to learn " << language << "!" << std::endl; return 0; 
}
```

![2025Fall-03-InitializationAndReferences, 页面 40](files/slides/CS106L/2025Fall-03-InitializationAndReferences.pdf#page=40&rect=0,0,960,540)

**A useful way to initialize some variables from data structures with fixed sizes at compile time**
**含义：一种从“编译期固定大小”的数据结构中初始化变量的便捷方法。**

**Can use on objects where the size is known at compile-time**
**含义：只能用于那些“在编译时就知道大小”的对象。**

*   **解析：**
    这是结构化绑定的**技术限制（硬性规则）**。
    编译器在编译代码时，必须确切知道要把右边的对象拆成**几个**变量。
    *   **✅ 适用的对象（大小固定）：**
        *   原生数组 (如 `int a[3]`)
        *   `std::pair` (大小固定为 2)
        *   `std::tuple` (定义时大小就定了)
        *   `struct` / `class` (成员变量数量是固定的)
        *   `std::array`
    *   **❌ 不适用的对象（大小动态）：**
        *   `std::vector` (运行时长度可变)
        *   `std::list`
    ...
    **为什么？**
    如果你写 `auto [x, y] = myVector;`，编译器会困惑：万一 `myVector` 运行时有 3 个元素怎么办？或者只有 1 个？因为无法确定，所以直接禁止。
...
class vector { 
	...
	vector(std::initializer_list<int> init){
        ...
        data = new int[mSize];
        int i = 0;
        for (const auto& val : init) { //注意,std::initializer_list没有实现[]运算符。
            data[i++] = val;
        }	 
	}
	// ... other methods ... 
