...
所以，为了改进性能，我们不共用一个 `diff` 了，我们搞3个`diff`（`float diff[3]`）。对应上一次，当前，以及下一次的iteration。
...
*   **分析**：
    ...
    *   因此，跑得快的线程可以安全地清零 `diff[(k+1)%3]`，**不存在竞态条件**。不需要等待其他线程，所以 Barrier 1 可以移除。而且修改代码后，Barrier 2 兼职了init的功能。
