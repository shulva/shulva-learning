...
being random. Write a program to find out how often a streak of six heads or a streak 
...
for experimentNumber in range(10000):
    ...
    for i in range(99):
        if list[i]==list[i+1]:
            count+=1
            if count%5==0:# 0%5 also equals to zero,so if you put the if out the big if , it will be wrong
                numberOfStreaks+=1
                break
        else:
            count=0
